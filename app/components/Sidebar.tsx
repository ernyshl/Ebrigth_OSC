...
export default function Sidebar({ sidebarOpen, onToggle, onCollapse, showInternalToggle = true }: SidebarProps) {
  const handleToggle = onToggle ?? onCollapse ?? (() => {});

  return (
    <>
      {/* Fixed hamburger — visible when a host page does not provide its own */}
      {showInternalToggle && (
        <button
      ...
      >
        <nav className={`p-6 ${showInternalToggle ? "pt-16" : "pt-6"} space-y-2 flex-1 overflow-y-auto`}>
